---
name: BDD Binder
description: Implement BDD step definitions by discovering selectors and validating the application DOM. Receive test plan from @bdd-planner, explore app to discover selectors, implement C# step bindings. Hand off to @bdd-healer for test failures.
---
...
Snapshots return YAML with element refs. Use these refs for click/fill/hover commands.
...
Search for step definition files in the workspace (e.g. `**/*.cs` under `**/StepDefinitions/**`).
Search for patterns in source files to find existing step bindings (`[Given]`, `[When]`, `[Then]`) and selector usage (`Locator`, `GetBy`, `css=`, `xpath=`).
...
2. `playwright-cli screenshot` — Verify page loaded
3. Check for authentication:
   - If login required → Ask user to sign in manually first
   - Confirm logged-in state before proceeding
4. `playwright-cli snapshot` — Examine page structure (returns YAML with element refs like e1, e5)
5. `playwright-cli eval "(...)()"` — Install click listener (once per session)
...
The click listener captures `event.target` and walks up the DOM tree. The first element in `hierarchy` is what was actually clicked (which may be an icon inside a button). Check `hierarchy[0].attrs` for data attributes before using aria-labels or text from `playwright-cli snapshot`.
...
**CRITICAL DECISION TREE - When click listener returns clickData:**
...
- `playwright-cli snapshot` - Find the UI element referenced in the step — note element refs
- Look for: data-testid, data-test, data-id, data-lp-id, or id attributes
- Note: element tag, visible text, surrounding context

**2. Test the Selector**
- If clickable: `playwright-cli click <ref>` then `playwright-cli eval "window.lastClick"` to get best selector
- If fillable: `playwright-cli snapshot` to identify input type and attributes
- If verifiable: `playwright-cli snapshot` to see the confirmation element

**3. Verify Post-Action State**
- After interaction: `playwright-cli snapshot` - What element appears/changes?
...
a) **Initial State Discovery**
   - `playwright-cli snapshot` - Note the trigger element (button, search icon, input field) and its ref
   - Document the selector for the trigger

b) **Open Dialog/Dropdown**
   - `playwright-cli click <ref>` on the trigger element
   - `playwright-cli snapshot` - Examine what appears (modal, flyout, dropdown panel)
...
c) **Selection Strategy Discovery**
   ...
   - `playwright-cli snapshot` to see the item structure
...
e) **Verify Dialog Closed**
   - `playwright-cli snapshot` after selection - Confirm dialog/dropdown closed
...
1. `playwright-cli snapshot` before interaction — note element refs
2. `playwright-cli click <ref>` to open
3. `playwright-cli snapshot` to see what appeared
4. Identify selection mechanism from YAML refs
5. `playwright-cli snapshot` after selection to confirm state
...
```csharp
// [BAD] NEVER USE - breaks with localization AND creates strict mode violations
...
// [OK] CORRECT - Always use data attributes from selector discovery
await Page.Locator("[data-id='nav-item-products']").ClickAsync();

// [OK] ACCEPTABLE - GetByText ONLY for assertions/waits on non-interactive content
...
```csharp
// [BAD] NEVER USE - These cause flaky tests
...
**ALWAYS DO THIS:**
```csharp
// 1. playwright-cli snapshot BEFORE action → Document available elements and refs
// 2. playwright-cli click/fill → Perform action
// 3. playwright-cli snapshot AFTER action → See what appeared
...
// Wait for page-specific element that appeared in snapshot
...
// Wait for panel that appeared in snapshot
...
// [BAD] Duplicate fallback without documentation
await Page.GetByRole(AriaRole.Link, new() { Name = "Product ABC" }).First.ClickAsync();
```
...
## Handoff to bdd-healer

When tests fail after implementing step definitions, mention @bdd-healer to hand off diagnosis of test failures for systematic diagnosis and repair.
...
## Custom Binding Generation for Unknown UI (Two-Tier)

When bdd-planner flags steps with `# Custom binding required`, this section applies.

### Page Type Detection Decision Tree

Before writing any custom binding, classify the page:

1. **Check URL** — `pagetype=` parameter
   - `entityrecord` / `entitylist` → Standard MDA, use `Support/Bindings/` (do NOT create custom)
   - `genux` → Generative page, create custom bindings
   - `custom` / `webresource` → Custom page, create custom bindings
   - No `pagetype` or unknown → Inspect DOM

2. **Check MDA data attributes** in DOM:
   - `[data-field-name]` on form controls → Standard form (Tier 1)
   - `[row-index]` + `[col-id]` on grid elements → Standard ag-Grid (Tier 1)
   - `[data-id="CommandBar"]` → Standard command bar (Tier 1)
   - **None of above present** → Non-standard (Tier 2), proceed with custom binding

### ARIA-First Inspection for Unknown UI

When page is Tier 2 (no MDA data attributes):

1. **`playwright-cli snapshot`** — Capture full ARIA tree
2. Identify interactive elements by role:
   - `role="grid"`, `role="row"`, `role="gridcell"` → Grid structure
   - `role="textbox"`, `role="combobox"` → Input fields
   - `role="button"` → Actions
   - `role="tab"`, `role="tabpanel"` → Tabbed navigation
3. Note accessible names (`name` property in ARIA tree) — these are your primary selectors
4. Check for any data-* attributes as secondary selectors

### Selector Strategy for ARIA-Only Pages

When no `data-testid`, `data-id`, or `id` attributes exist (common in generative pages):

1. **Primary: `GetByRole` with accessible name**
   ```csharp
   // Grid container
   var grid = Page.GetByRole(AriaRole.Grid);
   
   // Column header by name (stable — from Dataverse metadata)
   var header = Page.GetByRole(AriaRole.Columnheader, new() { Name = "Account Name" });
   
   // Rows by index (positional — no row-index attribute available)
   var rows = grid.GetByRole(AriaRole.Row);
   var targetRow = rows.Nth(index + 1); // +1 to skip header row
   
   // Cell within row
   var cells = targetRow.GetByRole(AriaRole.Gridcell);
   var targetCell = cells.Nth(columnIndex);
   ```

2. **Secondary: Structural containment**
   ```csharp
   // Scope interactions to a specific region
   var mainContent = Page.Locator("main");
   var gridInMain = mainContent.GetByRole(AriaRole.Grid);
   ```

3. **Fallback: Text content** (only for assertions, never for clicks)
   ```csharp
   await Expect(targetCell).ToHaveTextAsync("Expected Value");
   ```

### GenUX Grid Worked Example

A generative page grid (`pagetype=genux`) has this ARIA structure:
```
grid "Accounts"
  rowgroup
    row (header)
      ...
      columnheader "Phone"
    row
      gridcell "Contoso Ltd"
      gridcell "ACC-001"
      gridcell "555-0100"
    row
      gridcell "Fabrikam Inc"
      gridcell "ACC-002"
      gridcell "555-0200"
```

**No `row-index` or `col-id` attributes** — unlike standard ag-Grid views.

Custom binding implementation:
```csharp
[Binding]
public class GenUXGridSteps
{
    private readonly ScenarioContext _scenarioContext;
    private IPage Page => _scenarioContext.Get<IPage>("Page");

    public GenUXGridSteps(ScenarioContext scenarioContext)
    {
        _scenarioContext = scenarioContext;
    }

    [Then(@"I should see a grid with ""(.*)"" column")]
    public async Task ThenIShouldSeeGridWithColumn(string columnName)
    {
        var header = Page.GetByRole(AriaRole.Columnheader, new() { Name = columnName });
        await Expect(header).ToBeVisibleAsync();
    }

    [When(@"I select row (\d+) in the grid")]
    public async Task WhenISelectRowInGrid(int rowIndex)
    {
        var grid = Page.GetByRole(AriaRole.Grid);
        var rows = grid.GetByRole(AriaRole.Row);
        // Skip header row group — data rows start after headers
        var dataRow = rows.Nth(rowIndex + 1);
        await dataRow.ClickAsync();
    }

    [Then(@"the grid should display ""(.*)"" in row (\d+), column ""(.*)""")]
    public async Task ThenGridShouldDisplay(string expectedValue, int rowIndex, string columnName)
    {
        var grid = Page.GetByRole(AriaRole.Grid);
        ...
        Assert.IsTrue(colIndex >= 0, $"Column '{columnName}' not found in grid headers");

        var dataRow = grid.GetByRole(AriaRole.Row).Nth(rowIndex + 1);
        var cell = dataRow.GetByRole(AriaRole.Gridcell).Nth(colIndex);
        await Expect(cell).ToHaveTextAsync(expectedValue);
    }
}
```

### Custom Binding File Conventions

- **Location:** `StepDefinitions/` directory (NOT `Support/Bindings/` — that's frozen)
- **Naming:** `{FeatureArea}CustomSteps.cs` (e.g., `GenUXDashboardCustomSteps.cs`, `CanvasFormCustomSteps.cs`)
- **Namespace:** `{ProjectNamespace}.StepDefinitions`
- **One file per feature area** — don't mix unrelated custom bindings

### Pre-Flight Check

Before creating any custom binding:
1. **Check `guide_testing`** for existing step vocabulary — the pattern may already exist
2. **Search `Support/Bindings/`** — a standardized binding might cover the interaction
3. **Only create custom if neither exists** — avoid duplicating the frozen vocabulary
